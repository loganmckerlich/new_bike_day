...
def main() -> None:
    ...
    seg_meta_cols = ["segment_id", "name", "distance", "average_grade", "segment_type"]
    ...
    n_clean = len(cleaned)
    n_removed_mw = n_raw - len(
        apply_min_watts_filter(efforts_with_power, int(min_watts), descents_exempt=descents_exempt_watts)
    )
    # For descent removal count (before z-score, just the descent filter)
    _after_mw = apply_min_watts_filter(efforts_with_power, int(min_watts), descents_exempt=descents_exempt_watts)
    n_removed_desc = (
        int((_after_mw["segment_type"] == "descent").sum())
        if exclude_descents and "segment_type" in _after_mw.columns
        else 0
    )
    ...
    # ── Visualisation explainer ────────────────────────────────────────────────
    st.divider()
    st.subheader("🔬 How outlier detection works")
    st.markdown(
        "Pick any segment below to step through the outlier detection process "
        "with the current z-score threshold."
    )

    if cleaned.empty:
        st.info("No efforts remain after filtering. Adjust the parameters above.")
        st.stop()

    # Pick a segment that has at least 3 efforts with power
    _seg_effort_counts = (
        cleaned.groupby("segment_id")["effort_id"].count()
    )
    _seg_effort_counts = _seg_effort_counts[_seg_effort_counts >= 3]
    if _seg_effort_counts.empty:
        st.info("Not enough efforts per segment to illustrate outlier detection (need ≥ 3 per segment).")
        st.stop()
    ...
    _seg_options_named = [(sid, _seg_name_map.get(sid, str(sid))) for sid in _seg_options_sorted]

    example_seg_id: int = st.selectbox(
        "Example segment",
        options=[s[0] for s in _seg_options_named],
